...
if(isset($_SESSION['pcr'])){
    header('Location: pcr/pcrpatients.php');
}
?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
	...
	<body><!---->
		...
		<button class="add"> <a href="../pcr/Padd.php"><b>ADD</b></a>  </button> <!-- add patient button (once it is clicked on it will direct you to add form)-->
...
//first if search button has been clicked on  and an input(keyword) has been entered by the user and the input is not empty then perform the search
	if (isset($_POST['search']) && isset($_POST['keyword']) && ($_POST['keyword'] != "")){
      ...
      $keyword = "%$keywordfromform%";
			$itemDetailsSearchSql = 'SELECT * FROM pcrpatients where id like ? or name like ? ORDER BY id DESC';
        ...
        if($itemDetailsSearchStatement->rowCount() > 0){
						...
						while($row = $itemDetailsSearchStatement->fetch(PDO::FETCH_ASSOC)){
							...
							$output .= '<tr>' .
											...
											'<td>' . $row['result'] . '</td>' .
											'<td><a href="../pcr/delete.php?id='.$row["id"] .'"><img src="../images/delete.png"  width="18" height="20" style="margin-top:-10px; text-decoration:none;"/> </a>
											<a href="../pcr/edit.php?id='.$row["id"] .'"><img src="../images/edit.png"  width="18" height="20" style="margin-top:-10px; text-decoration:none;"/> </a>
                      <a href="../pcr/outputs/'.$row["id"].'.pdf"  target="_blank" style= "text-decoration:none;">
                      <img  src="../images/folder.png"  width="18" height="20" style="margin-top:-10px; text-decoration:none;"> </a></td>'.
										'</tr>';}
...
// here is the process of dividing the table in order to display 5 patients only in each page
	if (isset($_GET['page_no']) && $_GET['page_no']!="") {
        ...
        $result_count = mysqli_query($con,"SELECT COUNT(*) As total_records FROM pcrpatients");
				$total_records = mysqli_fetch_array($result_count);
				$total_records = $total_records['total_records'];
				$total_no_of_pages = ceil($total_records / $total_records_per_page);
				$second_last = $total_no_of_pages - 1; // total pages minus 1
				$result = mysqli_query($con,"SELECT * FROM pcrpatients ORDER BY id DESC LIMIT $offset, $total_records_per_page");
